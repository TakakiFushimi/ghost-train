...
from utils.training.Dataset_sub import SubFaceEmbedVGG2
...
def train_one_epoch(G: AEI_Net, 
                    ...
                    dataloader: torch.utils.data.DataLoader,
                    dataloader_sub: torch.utils.data.DataLoader,
                    device: torch.device,
                    epoch: int,
                    loss_adv_accumulated: int):
    """
    ...
    if epoch == args.start_epoch:
        ...
        dataloader_iter = dataloader_sub
        print(f'Skipping to batch: {start_batch}')
    else:
        start_batch = 0
        dataloader_iter = dataloader
    print(f'Dataloader iterator created: {dataloader_iter}')
    print(f'Dataloader iterator created: {len(dataloader_iter)}')
    print(f'Epoch: {epoch}')
    # データローダーの内容を確認
    dataloader_list = list(dataloader_sub)
    print(f'Dataloader content: {dataloader_list[:10]}')  # 最初の10バッチを確認
    for iteration, data in enumerate(dataloader_iter, start_batch):
...
def train(args, device):
    ...
    if intermediate== True:
        dataset_sub = SubFaceEmbedVGG2(args.dataset_path, same_prob=args.same_person, same_identity=args.same_identity, start_index=args.start_batch, end_index=193388)
        dataloader_sub = DataLoader(dataset_sub, batch_size=batch_size, shuffle=True, num_workers=8, drop_last=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--G_path', default='./weights/G_unet_2blocks.pth', help='Path to pretrained weights for G. Only used if pretrained=True')
    # Gの事前学習済み重みのパス。pretrained=Trueの場合にのみ使用されます。
    parser.add_argument('--D_path', default='./weights/D_unet_2blocks.pth', help='Path to pretrained weights for D. Only used if pretrained=True')
    ...
    parser.add_argument('--name', required=True, type=str, help='Name of this run. Used to create folders where to save the weights.')
    # この実行の名前。
    parser.add_argument('--run_id', default=None, type=str, help='Name of this run. Used to create folders where to save the weights.')
    # この実行のid。
    parser.add_argument('--wandb_project', default='GHOST', type=str)
    # wandbプロジェクトの名前
    parser.add_argument('--wandb_entity', default='fushissitakaki-japan', type=str)
    # wandbのエンティティ（ログイン名）
    parser.add_argument('--use_resume', default='must', type=str, help='Use wandb to track your experiments or not')
    # runを途中から再開する場合にTrue
    """
    ...
    if args.use_wandb == True:
        ...
        wandb.config.update({"dataset_path": args.dataset_path, "pretrained": args.pretrained, "G_path": args.G_path,"D_path": args.D_path}, allow_val_change=True)
