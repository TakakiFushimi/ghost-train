...
def train_one_epoch(rank, G, D, opt_G, opt_D, scheduler_G, scheduler_D, netArc, model_ft, args, dataloader, device, epoch, loss_adv_accumulated):
    for iteration, data in enumerate(dataloader):
        ...
        if iteration % args.show_step == 0 and rank == 0:
            images = [Xs, Xt, Y]
            if args.eye_detector_loss:
        ...
        if iteration % 5000 == 0 and rank == 0:
        ...
        if (iteration % 250 == 0) and (args.use_wandb) and rank == 0:
            G.eval()
...
def train(rank, world_size, args):
    ...
    G = DDP(G, device_ids=[rank], output_device=rank, find_unused_parameters=True)
    D = DDP(D, device_ids=[rank], output_device=rank, find_unused_parameters=True)
    netArc = DDP(netArc, device_ids=[rank], output_device=rank)
    ...
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=8, pin_memory=True, sampler=sampler, drop_last=True)

    loss_adv_accumulated = 20.
    
    for epoch in range(0, max_epoch):
        train_one_epoch(rank, G, D, opt_G, opt_D, scheduler_G, scheduler_D, netArc, model_ft, args, dataloader, device, epoch, loss_adv_accumulated)
